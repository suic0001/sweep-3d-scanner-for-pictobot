...
class ScanExporter(object):
    ...
    def export_2D_scan(self, scan, scan_index, angle_between_sweeps, mount_angle, CCW):
        ...
        h.fream_id = 'PointCloud2'
        pointcloud = PointCloud2()
        pointcloud.header = h

        for n, sample in enumerate(scan.samples):

            pointcloud.points[n].x = round(converted_coords[n, 0])
            pointcloud.points[n].x = round(converted_coords[n, 1])
            pointcloud.points[n].x = round(converted_coords[n, 2])

            #Below are original code to write a csv file
