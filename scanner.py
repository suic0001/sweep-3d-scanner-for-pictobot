...
def angular_range_callback(data):

    global angular_range
	
    angular_range = data.data
	
def set_ms_sr_ar():

    rospy.init_node('sub_ms_sr_ar')
	
    rospy.Subscriber("set_ms", std_msgs.msg.Int32, motor_speed_callback)
	
    rospy.Subscriber("set_sr", std_msgs.msg.Int32, sample_rate_callback)
	
    rospy.Subscriber("set_ar", std_msgs.msg.Int32, angular_range_callback)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-ms', '--motor_speed',#ask for motor speed input
                        help='Motor Speed (integer from 1:10)',
                        #default=sweep_helpers.MOTOR_SPEED_1_HZ,
                        required=False)
    parser.add_argument('-sr', '--sample_rate',#ask for sample rate input
                        help='Sample Rate(either 500, 750 or 1000)',
                        #default=sweep_helpers.SAMPLE_RATE_500_HZ,
                        required=False)
    parser.add_argument('-ar', '--angular_range',#ask for scan range  
    ...
    while not rospy.is_shutdown():
        if motor_speed is not None and sample_rate is not None and angular_range is not None:

            argsdict['motor_speed'] = motor_speed
            argsdict['sample_rate'] = sample_rate
            argsdict['angular_range'] = angular_range
